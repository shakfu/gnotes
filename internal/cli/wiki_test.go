...
func wikiFixture(t *testing.T) *fixture {
	...
	if out := f.mustRun("wiki", "init"); !strings.Contains(out, "0 pages") {
		t.Fatalf("init: %s", out)
	}
	pages := filepath.Join(f.dir, ".gnotes", "wiki")
...
func TestWikiListShowAndSearch(t *testing.T) {
	...
	if out := f.mustRun("wiki", "ls", "-t", "design"); strings.Count(out, "\n") != 1 {
		t.Errorf("ls -t design:\n%s", out)
	}

	out = f.mustRun("wiki", "show", "design", "sketch")
	for _, want := range []string{"Design sketch  .gnotes/wiki/lexer/design-sketch.md", "tokenizes input", "[[Missing page]]", "missing-page", "backlinks", "index:3"} {
		if !strings.Contains(out, want) {
	...
	out = f.mustRun("wiki", "search", "tokeniz")
	if !strings.Contains(out, "lexer/design-sketch  Design sketch") || !strings.Contains(out, "tokenizes") {
		t.Errorf("search:\n%s", out)
	}
	if out := f.mustRun("wiki", "search", "no such words"); !strings.Contains(out, "nothing matches") {
...
func TestWikiLinksCheckAndExitStatus(t *testing.T) {
	...
	out := f.mustRun("wiki", "links", "index")
	if !strings.Contains(out, "lexer/grammar.md#rules") || strings.Contains(out, "missing") {
		t.Errorf("links index:\n%s", out)
	}
	if out := f.mustRun("wiki", "backlinks", "lexer/grammar.md"); !strings.Contains(out, "index:3") {
		t.Errorf("backlinks:\n%s", out)
	}

	out, stderr, code := f.run("wiki", "check")
	...
	if err := os.WriteFile(filepath.Join(f.dir, ".gnotes", "wiki", "missing-page.md"), []byte("# Missing page\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	if out := f.mustRun("wiki", "check"); !strings.Contains(out, "no broken links") {
...
func TestWikiTasksOrphansAndCache(t *testing.T) {
	...
	out := strings.Join(strings.Fields(f.mustRun("wiki", "tasks")), " ")
	if !strings.Contains(out, "lexer/design-sketch:8 [ ] 2026-08-21 benchmark") || !strings.Contains(out, "tasks/ship [~] Ship it") {
		t.Errorf("tasks:\n%s", out)
	}
	if out := f.mustRun("wiki", "tasks", "-s", "doing"); strings.Contains(out, "benchmark") || !strings.Contains(out, "Ship it") {
		t.Errorf("tasks -s doing:\n%s", out)
	}
	if _, _, code := f.run("wiki", "tasks", "-s", "later"); code != 2 {
		t.Errorf("tasks -s later: exit %d, want 2", code)
	}

	out = f.mustRun("wiki", "orphans")
	if !strings.Contains(out, "tasks/ship") || strings.Contains(out, "lexer/grammar") {
		t.Errorf("orphans:\n%s", out)
	}

	if out := f.mustRun("wiki", "cache", "--rebuild"); !strings.Contains(out, "5 pages") {
...
func TestWikiJSON(t *testing.T) {
	...
	for _, args := range [][]string{
		{"wiki", "ls", "--json"},
		{"wiki", "show", "grammar", "--json"},
		{"wiki", "search", "lexer", "--json"},
		{"wiki", "links", "index", "--json"},
		{"wiki", "backlinks", "grammar", "--json"},
		{"wiki", "orphans", "--json"},
		{"wiki", "tasks", "--json"},
	} {
		out := f.mustRun(args...)
		if !json.Valid([]byte(out)) {
			t.Errorf("gnotes %s is not JSON:\n%s", strings.Join(args, " "), out)
		}
		if strings.ContainsAny(out, "\x02\x03") {
			t.Errorf("gnotes %s leaked snippet markers", strings.Join(args, " "))
		}
	}
	out, _, code := f.run("wiki", "check", "--json")
...
func TestWikiErrors(t *testing.T) {
	f := wikiFixture(t)
	if _, _, code := f.run("wiki", "nosuch"); code != 2 {
		t.Errorf("unknown subcommand: exit %d, want 2", code)
	}
	if out := f.mustRun("wiki"); !strings.Contains(out, "backlinks <page>") {
		t.Errorf("bare wiki does not list its commands:\n%s", out)
	}
	if _, stderr, code := f.run("-g", "wiki", "ls"); code == 0 || !strings.Contains(stderr, "-g") {
		t.Errorf("-g wiki ls: exit %d, %s", code, stderr)
	}
	if _, stderr, code := f.run("wiki", "show", "sketch", "grammar"); code == 0 || !strings.Contains(stderr, "no page matches") {
		t.Errorf("show of nothing: exit %d, %s", code, stderr)
	}
	if _, stderr, _ := f.run("wiki", "show", "lexer"); !strings.Contains(stderr, "could mean") {
		t.Errorf("ambiguous show: %s", stderr)
	}

	outside := &fixture{t: t, dir: t.TempDir(), now: f.now}
	if _, stderr, code := outside.run("wiki", "ls"); code == 0 || !strings.Contains(stderr, "gnotes wiki init") {
...
func TestWikiDoesNotPrintControlCharacters(t *testing.T) {
	f := wikiFixture(t)
	for _, args := range [][]string{{"wiki", "ls"}, {"wiki", "show", "evil-title"}, {"wiki", "orphans"}, {"wiki", "search", "evil"}} {
		out := f.mustRun(args...)
...
func TestWikiNewEditAndTag(t *testing.T) {
	...
	out := f.mustRun("wiki", "new", "Parser notes", "--in", "lexer", "-t", "parser", "-m", "First line.")
	if !strings.Contains(out, ".gnotes/wiki/lexer/parser-notes.md  Parser notes") {
		t.Fatalf("new: %s", out)
	}
	if out := f.mustRun("wiki", "new", "Ship the parser", "--task"); !strings.Contains(out, "ship-the-parser.md") {
		t.Fatalf("new --task: %s", out)
	}
	if out := strings.Join(strings.Fields(f.mustRun("wiki", "tasks", "-s", "open")), " "); !strings.Contains(out, "ship-the-parser [ ] Ship the parser") {
		t.Fatalf("the new task page is not listed:\n%s", out)
	}

	f.mustRun("wiki", "edit", "parser notes", "-m", "Replaced.")
	page := readPage(t, f, "lexer/parser-notes")
	if page != "---\ntags: [parser]\n---\n\nReplaced.\n" {
		t.Fatalf("edit -m:\n%s", page)
	}
	f.runIn("From stdin.\n", "wiki", "edit", "parser notes", "--stdin")
	...
	f.mustRun("wiki", "edit", "parser notes")
	...
	if out := f.mustRun("wiki", "tag", "lexer/parser-notes", "Lexer", "#extra"); !strings.Contains(out, "#extra #lexer #parser") {
		t.Fatalf("tag: %s", out)
	}
	...
	if out := f.mustRun("wiki", "untag", "lexer/parser-notes", "parser"); !strings.Contains(out, "#extra #lexer") || strings.Contains(out, "#parser") {
...
func readPage(t *testing.T, f *fixture, id string) string {
	t.Helper()
	raw, err := os.ReadFile(filepath.Join(f.dir, ".gnotes", "wiki", filepath.FromSlash(id)+".md"))
	if err != nil {
		t.Fatal(err)
	}
	return string(raw)
...
func TestWikiMoveAndRemove(t *testing.T) {
	...
	out := f.mustRun("wiki", "mv", "lexer/grammar", "archive/", "--dry-run")
	...
	if _, err := os.Stat(filepath.Join(f.dir, ".gnotes", "wiki", "archive", "grammar.md")); !os.IsNotExist(err) {
		t.Fatal("--dry-run moved the page")
	}

	out = f.mustRun("wiki", "mv", "lexer/grammar", "archive/")
	...
	if out := f.mustRun("wiki", "links", "index"); strings.Contains(out, "missing") {
		t.Fatalf("links after mv:\n%s", out)
	}

	_, stderr, code := f.run("wiki", "rm", "archive/grammar")
	if code != 1 || !strings.Contains(stderr, "links reach") {
		t.Fatalf("rm of a linked page: exit %d, %s", code, stderr)
	}
	if out := f.mustRun("wiki", "rm", "archive/grammar", "--force"); !strings.Contains(out, "removed .gnotes/wiki/archive/grammar.md") {
...
func TestWikiTasksCommands(t *testing.T) {
	...
	if out := f.mustRun("wiki", "done", "benchmark"); !strings.Contains(out, "lexer/design-sketch:8  done") {
		t.Fatalf("done: %s", out)
	}
	if page := readPage(t, f, "lexer/design-sketch"); !strings.Contains(page, "- [x] benchmark") {
		t.Fatalf("item not ticked:\n%s", page)
	}
	f.mustRun("wiki", "reopen", "lexer/design-sketch:8")
	if _, stderr, code := f.run("wiki", "doing", "benchmark"); code == 0 || !strings.Contains(stderr, "promote") {
		t.Fatalf("doing on an item: exit %d, %s", code, stderr)
	}
	f.mustRun("wiki", "done", "Ship it")
	if page := readPage(t, f, "tasks/ship"); !strings.Contains(page, "status: done") {
		t.Fatalf("task page:\n%s", page)
	}

	if out := f.mustRun("wiki", "promote", "benchmark"); !strings.Contains(out, "tasks/benchmark.md  benchmark  from lexer/design-sketch:8") {
...
func TestWikiCheckFix(t *testing.T) {
	...
	f.mustRun("wiki", "new", "Missing pages")
	f.mustRun("wiki", "new", "Missing paper")

	// --fix=first applies the only offer for the file; [[Missing page]] has
	// two similar pages, so it is left.
	out, _, code := f.runIn("", "wiki", "check", "--fix=first")
...
{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"gnotes_read","arguments":{"page":"grammar"}}}
`
	stdout, stderr, code := f.runIn(frames, "wiki", "mcp")
	if code != 0 {
